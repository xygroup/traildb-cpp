...
#include "traildb-cpp/TrailDBcpp.h"
...
int main() {
  ...
  for (std::uint32_t num = 0; num < num_events; ++num) {
    ...
    for (evtIter = eventList->EventsBegin(); evtIter != eventList->EventsEnd();
         ++evtIter) {
      std::uint32_t ts = evtIter->GetTimestamp();

      std::cout << tdb.GetHexUUIDByInd(num) << " " << ts << " "
                << evtIter->GetTrail("field1") << " "
