...
class Event {
 public:
  Event(std::uint32_t timestamp = 0xFFFFFFFFULL) : timestamp_(timestamp) {}
  ...
  void ResetTrail(std::string item) { trail_[item] = ""; }
  ...
  bool IsEventType(std::string type) { return GetTrail("type") == type; }

  std::string GetEventType() { return GetTrail("type"); }
...
class EventList {
 public:
  ...
  EventList(int nReserve) { events_.reserve(nReserve); }
  ...
  std::uint32_t GetCountEventType(std::string type) {
    std::uint32_t count = (std::uint32_t)count_if(
        EventsBegin(), EventsEnd(), bind2nd(EventTypeMatch(), type));
    return count;
  }
...
class TrailDB {
 public:
  ...
  std::vector<std::string> GetDimNames() const { return fieldNames_; }
...
TrailDB::TrailDB(std::string dbpath) : dbPath_(dbpath) {
  db_ = tdb_init();
...
std::uint32_t TrailDB::GetNumberOfEvents(std::uint64_t uuid_idx) {
  ...
  if (tdb_get_trail(cursor, uuid_idx)) throw TrailDBException();
